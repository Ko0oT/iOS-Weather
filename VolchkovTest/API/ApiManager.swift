...
final class ApiManager {
    ...
    func getWeather(for city: String, completion: @escaping (WeatherOfCity) -> Void) {
        let request = "\(self.baseURL)weather?q=\(city)&appid=\(self.apiKey)&units=metric&lang=ru"
        guard let url = URL(string: request) else { return }
        let task = URLSession.shared.dataTask(with: url) { data, responce, error in
            if let data = data, let weather = try? JSONDecoder().decode(WeatherOfCity.self, from: data) {
                completion(weather)
